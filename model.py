...
class PositionalEncoding(nn.Module):
  def __init__(self, d_model:int, seq_length: int, dropout: float = 0.1, device: torch.device = torch.device('cpu')):
    super().__init__()
    self.d_model = d_model
...
class MultiheadAttention(nn.Module):
  def __init__(self, d_model: int, num_heads: int, dropout: float = 0.1):
    ...
    self.d_v = self.d_model // self.num_heads # Dimension of vector seen by each head
  ...
  def forward(self, q, k, v, mask):
    ...
    query = query.view(query.shape[0], query.shape[1], self.num_heads, self.d_v).permute(0,2,1,3)
    key = key.view(key.shape[0], key.shape[1], self.num_heads, self.d_v).permute(0,2,1,3)
    value = value.view(value.shape[0], value.shape[1], self.num_heads, self.d_v).permute(0,2,1,3)
    ...
    x = x.permute(0,2,1,3).contiguous().view(x.shape[0], -1, self.num_heads * self.d_v)
...
class EncoderBlock(nn.Module):
  def __init__(self, d_model, self_attention_block: MultiheadAttention, feedforward_block: FeedForwardBlock, dropout: float = 0.1):
    super().__init__()
    self.d_model = d_model
...
class DecoderBlock(nn.Module):
  def __init__(self, d_model: int, self_attention_block: MultiheadAttention, cross_attention_block: MultiheadAttention, feedforward_block: FeedForwardBlock, dropout: float):
    super().__init__()
    self.d_model = d_model
    self.self_attention_block = self_attention_block
  ...
  def forward(self, x, encoder_output, src_mask, tgt_mask):
    ...
    x = self.residual_connection_2(x, lambda x: self.cross_attention_block(x, encoder_output, encoder_output, tgt_mask))
...
class Transformer(nn.Module):
  def __init__(self, d_model: int, encoder: Encoder, decoder: Decoder, src_embedding: EmbeddingLayer, tgt_embedding: EmbeddingLayer, projection_layer: ProjectionLayer, src_pe: PositionalEncoding, tgt_pe: PositionalEncoding):
    super().__init__()
    self.d_model = d_model
